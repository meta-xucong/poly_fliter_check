    ...
    for params in search_params:
        ...
        for item in events:
            ...
            if key and key not in seen:
                seen[key] = EventData(event_id=event_id or key, slug=event_slug or slug, question=question, markets=markets)
...
def fetch_historical_events(keywords: List[str]) -> List[EventData]:
    if not keywords:
        return []
    search_phrase = " ".join(keywords)
    data = _fetch_json(f"{GAMMA_HOST}/events", params={"search": search_phrase, "closed": True})
    events: Iterable[Dict] = data if isinstance(data, list) else data.get("events", [])
    ...
    for item in events:
        ...
        markets = item.get("markets") or []
        # 关键词全部命中 slug 即视为相关历史盘
        slug_lower = (event_slug or "").lower()
        if all(k in slug_lower for k in keywords):
            results.append(EventData(event_id=event_id or event_slug, slug=event_slug, question=question, markets=markets))
    return results
...
def main() -> None:
    ...
    historical_events = fetch_historical_events(keywords)
